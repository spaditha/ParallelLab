 ...
 */
public class AmazonTop100PositiveReviewers extends Configured implements Tool {
	// Just used for logging
	protected static final Logger LOG = LoggerFactory.getLogger(AmazonTop100PositiveReviewers.class);
	
	// This is the execution entry point for Java programs
	public static void main(String[] args) throws Exception {
		int res = ToolRunner.run(HBaseConfiguration.create(), new AmazonTop100PositiveReviewers(), args);
		System.exit(res);
	}
	...
	public int run(String[] args) throws Exception {
		...
		Job job = Job.getInstance(getConf(), "AmazonTop100PositiveReviewers");
		job.setJarByClass(AmazonTop100PositiveReviewers.class);
		...
		TableMapReduceUtil.initTableMapperJob(
			...
			AverageReviews.class,	// Mapper output value
			job,				// This job
		...
		// Specifies the reducer class to used to execute the "reduce" method after "map"
    		...
    		job.setOutputValueClass(AverageReviews.class);
    
    		// What for the job to complete and exit with appropriate exit code
		return job.waitForCompletion(true) ? 0 : 1;
	}

	public static class AverageReviews implements Writable {
	...
	public static class MapReduceMapper extends TableMapper<Text, AverageReviews> {
		...
		private AverageReviews reviewerAverage = new AverageReviews();
		...
		protected void setup(Context context) {
			...
			rowsProcessed = context.getCounter("AmazonTop100PositiveReviewers", "Rows Processed");
    		}
	...
	public static class MapReduceReducer extends Reducer<Text, AverageReviews, Text, AverageReviews> {
		
		private AverageReviews result = new AverageReviews();

		@Override
		public void reduce(Text key, Iterable<AverageReviews> values, Context context) throws IOException, InterruptedException {
			double sum = 0;
			long count = 0;
			for (AverageReviews reviewerAverage : values) {
				//sum += count.get();
